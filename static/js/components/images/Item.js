...
const Item = ({ id, type, active = 0 }) => {
  const tips = useContext(TipContext);
  const tip =
    tips.item && tips.item[id] ? (
      <div dangerouslySetInnerHTML={{ __html: tips.item[id] }} />
    ) : (
  ...
  return (
    <Tippy
      content={tip}
      onShow={() => {
        if (!tips.showItem) tips.setShowItem(true);
      }}
      maxWidth={620}
    >
      <img
        className={styles[type + (active ? active : "")]}
        src={`https://cdn5.lolalytics.com/item64/${id}.webp`}
        alt={alt[id]}
        data-id={id}
      />
    </Tippy>
