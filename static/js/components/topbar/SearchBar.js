...
const SearchBar = (({nav}) => {
  ...
  useEffect(()=>{
    if (tips.lang!=='eng') {
      if(tips.translation.champions) setLocalChamps(tips.translation.champions)
    }
    else setLocalChamps(false)
  },[tips.lang,tips.translation])
  ...
  return (
    <div ref={searchRef} className={styles.search}>
      ...
      <input ref={inputRef} placeholder={'Search Champion Builds & Profiles'} value={value} tabIndex="0" autoComplete="off" spellCheck="false" onChange={handleChange} onKeyDown={(e) =>handleKeyDown(e,tier,mode,patch)} />
        ...
        { results.length>0 && results.map((result,i)=>
          <div className={styles.row} key={i} >
            <div className={(cid <1) ? styles.fullwidth : ''}>
              <ChampionLink cid ={result[0]} nav={nav}><div><Champ id={result[0]} type="champ32b" toolTip={false} /> <ChampionName cid={result[0]} /> Build</div></ChampionLink>
              { cid >0 && result[0] !== cid &&
                <ChampionVsLink cid={cid} vs={result[0]} nav={nav}><div><Champ id={cid} type="champ32b" toolTip={false} /> vs <Champ id={result[0]} type="champ32b" toolTip={false} /></div></ChampionVsLink>
              }
            </div>
            <div className={styles.sublinks}>
              <ChampionLink cid ={result[0]} page="leaderboard" nav={nav}><div><Trans trans="tabs::main::leaderboard" english="Leaderboard" maxChars={11} /></div></ChampionLink>
              <ChampionLink cid ={result[0]} page="counters" nav={nav}><div><Trans trans="tabs::main::counters" english="Counters"/></div></ChampionLink>
        ...
        <div className={styles.row}>
          <div className={styles.fullwidth}>
            <a href={summonerURL(regions[selector],value)}>
              <div className={styles.summoner}><div className={styles[regions[selector]]}>{regions[selector]}</div>{value}</div>
            </a>
          </div>
        </div>
